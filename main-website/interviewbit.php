...
<html data-bs-theme="light" lang="en">
    <head>
        ...
        <title>Interviewbit | CodersScores</title>
    ...
    <div class="container py-md-5">
        <div class="row">
            <div class="col-md-6 text-center">
                <img class="img-fluid w-100" src="assets/img/interviewbit-logos-idw2RlQgVg.svg" width="476" height="248" style="padding-bottom: 0px;margin-bottom: 0px;padding-top: 0px;margin-top: 36px;">
            </div>
            <div class="col-md-5 col-xl-4 text-center text-md-start">
                <h2 class="display-6 fw-bold mb-5"></h2>
                <form method="post" data-bs-theme="light">
                    <div class="mb-3">
                    <input class="form-control" type="text" name="uname" placeholder="Interviewbit user name" required="">
                </div>
                <div class="mb-5">
...
if (isset($_POST["uname"])){
    ...
    $url='https://www.interviewbit.com/profile/'.$username.'/';
    $headers = @get_headers($url);
    if ($headers && strpos($headers[0], '200') !== false) {
    include("simple_html_dom.php");
    $html = file_get_html($url);
   echo($html->find('.profile-overview-user-details__name',0)->plaintext);
    $name = $html->find('h3.name', 0)->plaintext;
    $rank = toNum($html->find('.txt', 0)->plaintext);
    $s = toNum($html->find('.txt', 1)->plaintext);
    $st = toNum($html->find('.txt', 2)->plaintext);
    $problems=$html->find('.profile-daily-goal__goal-details',0)->plaintext;
